...
def _no_grad_uniform_(tensor, a, b):
    with torch.no_grad():
        tensor.copy_(
            torch.uniform(shape=tensor.shape, dtype=tensor.dtype, min=a, max=b)
        )
        return tensor
...
def _no_grad_normal_(tensor, mean=0.0, std=1.0):
    with torch.no_grad():
        tensor.copy_(torch.normal(mean=mean, std=std, shape=tensor.shape))
        return tensor
...
def _no_grad_trunc_normal_(tensor, mean=0.0, std=1.0, a=2.0, b=2.0):
    ...
    with torch.no_grad():
        ...
        epsilon = 1e-6
        _tensor = torch.rand(tensor.shape) * (2 * u - 1 - (2 * l - 1) + epsilon) + (2 * l - 1)
        ...
        _tensor = torch.multiply(_tensor, torch.tensor(std * math.sqrt(2.0)))
        _tensor = torch.add(_tensor, torch.tensor(mean))

        # Clamp to ensure it"s in the proper range
        _tensor = torch.clip(_tensor, min=a, max=b)
...
def uniform_(tensor: torch.Tensor, a: float, b: float) -> torch.Tensor:
    ...
    Examples:
        >>> import torch
        >>> import ppsci
        >>> param = torch.empty((128, 256), "float32")
...
def constant_(tensor: torch.Tensor, value: float = 0.0) -> torch.Tensor:
...
def xavier_uniform_(
    tensor: torch.Tensor, gain: float = 1.0, reverse: bool = False
) -> torch.Tensor:
...
def linear_init_(module: nn.Module) -> None:
    """Initialize module's weight and bias as it is a linear layer.

    Args:
        module (nn.Module): Linear Layer to be initialized.
    """
...
def conv_init_(module: nn.Module) -> None:
    """Initialize module's weight and bias as it is a conv layer.

    Args:
        module (nn.Module): Convolution Layer to be initialized.
    """
