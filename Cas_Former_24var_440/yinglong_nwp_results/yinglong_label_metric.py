...
for d in tqdm(valid_days, desc="Computing yearly averages"):
    ...
    # RMSE（物理空间，直接累加MSE）
    sum_rmse_phy += np.mean((fcst_c - truth_c) ** 2, axis=(-2, -1))
    ...
    # 直接广播计算ACC
    acc_day = np.sum((fcst_norm - mean_norm_c) * (truth_norm - mean_norm_c), axis=(-2, -1)) / \
...
# 计算年平均
mean_rmse_phy = np.sqrt(sum_rmse_phy / n_days_used).astype(np.float32)
mean_acc = (sum_acc / n_days_used).astype(np.float32)

# 创建输出目录
os.makedirs(output_dir, exist_ok=True)

# 保存结果
np.save(os.path.join(output_dir, "rmse_yinglong.npy"), mean_rmse_phy)
np.save(os.path.join(output_dir, "acc_yinglong.npy"), mean_acc)
