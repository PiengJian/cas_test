...
from hrrr_dataset_marge import HRRRDataset

from tools import save_tools
from loss.l2 import RelativeL2Loss
import datetime
from models.afno_attention_parallel_marge import AFNOAttnParallelNet
...
Wm=torch.from_numpy(np.load('./mwp67.npy')).float()
Wn=torch.from_numpy(np.load('./nwp67.npy')).float()


marge_mean = torch.from_numpy(data_mean).float().to(device=device)
marge_std = torch.from_numpy(data_std).float().to(device=device)

Wm = Wm.to(device=device)
Wn = Wn.to(device=device)
...
with torch.no_grad():
    ...
    # 每一个iteration
    for i, (input_item, label_item, weight_item,input_time,label_time, geo, marge) in enumerate(dataloader_):
        ...
        out = model(input_item, input_time, geo,  marge, marge_mean,marge_std,Wm,Wn)
        ...
        if(i%10==0 and i!=0):
            ...
            with open(log_file_path, 'a') as log_file:
                ...
                acc = ACC_list/(i+1)
                np.save(OUTPUT_DIR + "/rmse_marge.npy", rmse)
                np.save(OUTPUT_DIR + "/acc_marge.npy", acc)
