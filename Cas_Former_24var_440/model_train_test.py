...
class ModelTrain():
    ...
    def train_one_epoch(self,epoch):
        self.model.num_timestamps = self.train_label_timestamps
        self.model.train()
        ...
        geo = geo.contiguous().reshape((self.train_batch_size, 1 , self.IMG_H, self.IMG_W)).to(self.device)
    
    
        # for batch_idx, (input_item, label_item,input_time,label_time) in enumerate(self.train_loader):
        batch_idx, (input_item, label_item,input_time,label_time) = 0,next(iter(self.train_loader))
        ...
        if input_item.shape[0] != self.train_batch_size:
            ...
            geo = torch.from_numpy(geo).to(torch.float32)
            geo = geo.contiguous().reshape((input_item.shape[0], 1 , self.IMG_H, self.IMG_W)).to(self.device)
    ...
    def test_one_epoch(self,merge,epoch):
        ...
        # 加载30个变量的统计信息
        mean_all = np.load("/nfs/samba/数据聚变/气象数据/hrrr_440_30var/traindata_stat/var_pos_mean.npy")[:,-224:,:224].reshape(1, 30, self.IMG_H, self.IMG_W).astype(np.float32)
        ...
        if merge:
            Wm=torch.from_numpy(np.load('/home/pengjian/boundary_weight/mwp32.npy')).float().to(self.device)
            Wn=torch.from_numpy(np.load('/home/pengjian/boundary_weight/nwp32.npy')).float().to(self.device)
        ...
        with torch.no_grad():
            if merge:
                ...
                i,(input_item, label_item,input_time,label_time, merge) =10,next(iter(self.test_loader))
                ...
                i,(input_item, label_item,input_time,label_time) =10,next(iter(self.test_loader))
