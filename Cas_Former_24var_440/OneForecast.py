...
'''
它不同于简单地把 src_feat ∥ dst_feat ∥ efeat 拼接后再喂入一个大 MLP，
而是把三部分各自用一层线性变换处理后再直接相加（sum trick），然后再接入一个小型 MLP 做非线性映射与归一化。
这样做能减少拼接带来的参数膨胀，同时与拼接 + MLP 保持等价的表达能力。
...
def get_icosahedron() -> TriangularMesh:
    ...
    '''
    vertices 是一个 numpy 数组，用于存储网格中所有顶点的三维坐标。
    faces 也是一个 numpy 数组，用于存储构成三角形面的顶点索引。每个面由三个顶点索引组成，这三个顶点按顺序连接形成一个三角形。
    '''
    return TriangularMesh(
