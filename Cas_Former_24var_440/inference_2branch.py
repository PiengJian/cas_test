...
from metric.RMSE_2branch import RMSE, ACC
...
father_path = "/nfs/samba/数据聚变/气象数据/hrrr区域划分实验数据/hrrr_440_24var_lb_mountain"
...
#pretrain_model  = torch.load('./output/rb/pretrain/modelww_27.pth')

pretrain_model  = torch.load('./output/rb/finetune/modelww_6.pth')
...
RMSE_list1= [[0 for i in range(48)] for i in range(24)]
ACC_list1= [[0 for i in range(48)] for i in range(24)]

RMSE_list2= [[0 for i in range(48)] for i in range(24)]
ACC_list2= [[0 for i in range(48)] for i in range(24)]
total_loss = 0
...
with torch.no_grad():
    ...
    for i, (input_item, label_item, weight_item,input_time,label_time, geo) in enumerate(dataloader_):
        ...
        RMSE_list1, RMSE_list2 = RMSE(out, label_list, RMSE_list1, RMSE_list2)
        # ACC_list = ACC(out_list[1:], target, RMSE_list)


        ACC_list1, ACC_list2 = ACC(out, label_list, ACC_list1, ACC_list2,mean,data_mean,data_std)
        ...
        if(i%10==0 and i!=0):
            ...
            with open(log_file_path, 'a') as log_file:
                ...
                rmse1 = (RMSE_list1/(i+1))*std
                rmse2 = (RMSE_list2/(i+1))*std
                acc1 = ACC_list1/(i+1)
                acc2 = ACC_list2/(i+1)
                np.save(OUTPUT_DIR + "/rmse_w.npy", rmse1)
                np.save(OUTPUT_DIR + "/rmse_e.npy", rmse2)
                np.save(OUTPUT_DIR + "/acc_w.npy", acc1)
                np.save(OUTPUT_DIR + "/acc_e.npy", acc2)
